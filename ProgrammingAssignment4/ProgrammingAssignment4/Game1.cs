        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            else if (mouse.RightButton == ButtonState.Released)
            {
                ...
                if (rightClickStarted)
                {
                    ...
                    var targetLocation = new Vector2(mouse.X, mouse.Y);
                    pickups.Add(new Pickup(pickupSprite, targetLocation));

                    // STUDENTS: if this is the first pickup in the list, set teddy target
                    if (pickups.Count == 1) teddy.SetTarget(targetLocation);
                }
            }
                 ...
                 teddy.CollisionRectangle.Intersects(pickups[0].CollisionRectangle))
            {
                ...
                if (pickups.Any())
                {
                    var rect = pickups.First().CollisionRectangle;
                    teddy.SetTarget(new Vector2(rect.X, rect.Y));
                }
                else
                {
